...
#sends listed commands via send_string_and_wait_for_string to the device connected to via dev_connect
	...
	send_string_and_wait_for_string(r"copy flash:/" + local_file + r" tftp://" + tftp_server + r"/IOS/" + switch['hostname'] + ".txt\n", "#", False)
	send_string_and_wait_for_string("conf t\n", "(config)#", False)
...
with open(r'C:\Scripts\switches-Cisco.csv','r') as f:
	for line in f:
		switch = {}
